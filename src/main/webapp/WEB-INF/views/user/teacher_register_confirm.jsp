...
<title>OICareerLog | 教員情報確認</title>
</head>
			...
			<div class="card shadow-sm border-0">
				<div class="card-body p-4">
					<dl class="row mb-4">
						...
						<dt class="col-4 text-muted">区分</dt>
						<dd class="col-8 fw-semibold">
							<c:choose>
								<c:when test="${teacherType == 'homeroom'}">
									担任教員（クラス情報の登録）
								</c:when>
								<c:otherwise>
									その他教員・職員
						...
						<c:if test="${teacherType == 'homeroom'}">
							<dt class="col-4 text-muted">担当学科</dt>
							<dd class="col-8 fw-semibold">${selectedDepartment.departmentName}</dd>
							<dt class="col-4 text-muted">担当学年</dt>
							<dd class="col-8 fw-semibold">${selectedGrade}年生</dd>
						</c:if>
					</dl>
					<div class="d-flex flex-column flex-md-row gap-3">
						<c:url var="backUrl" value="/NewTeacher">
							<c:param name="teacherType" value="${teacherType}" />
							<c:if test="${teacherType == 'homeroom'}">
						...
						<a class="btn btn-outline-secondary flex-fill" href="${backUrl}">
							入力画面に戻る </a>
						<form method="post" action="${contextPath}/NewTeacher"
							class="flex-fill d-grid gap-3">
							<input type="hidden" name="action" value="register" />
							<input type="hidden" name="teacherType" value="${teacherType}" />
							<c:if test="${teacherType == 'homeroom'}">
